...
export default function Navbar() {
  ...
  return (
    <>
      <AppBar position="sticky" sx={{ background: "rgba(18,18,26,0.95)", backdropFilter: "blur(10px)", borderBottom: "1px solid rgba(239,83,80,0.2)" }}>
        <Toolbar>
          {/* Logo */}
          <SecurityIcon sx={{ color: "primary.main", mr: 1 }} />
          <Typography variant="h6" fontWeight={700} sx={{ flexGrow: 1, cursor: "pointer", color: "white" }} onClick={() => navigate("/")}>
          ...
          {!isMobile && navLinks.map((link) => (
            <Button
              ...
              sx={{
                color: location.pathname === link.path ? "primary.main" : "grey.300",
